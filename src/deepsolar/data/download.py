# -*- coding: utf-8 -*
import os
from deepsolar.data.wrappers import GoogleMapsAPIDownloader, GoogleMapsWebDownloader, Sentinel2Downloader
from deepsolar.data.utils import compute_centers
...
def download_sent2(folder):
    ...
    api.request_products(mission="S2A", level="MSIL2A", baseline_no="*", relative_orbit_no="R094", tile="T30TVK", year=2021)
...
def download_gmaps_web(places, folder="../../data"):
    ...
    for name, coords in places.items():
        path = f"{folder}/{name}"
        gmaps.download(*coords, folder=path, zoom=19, style="s", format="png")
